<template>
  <div class="home">
    ...
    <!-- 导航 -->
    <Row class="pt-3">
        <Col :span="4" v-for="(item,index) in mix_116" :key="index" class="ml-2"><img :src="item.pic" alt="" style="width:60%;"></Col>
    </Row>
    <Row class="pb-2">
        <Col :span="4" v-for="(item,index) in mix_116" :key="index" class="ml-2 font-12">{{item.title}}</Col>
    </Row>
    ...
    <Row>
        <Col :span="8" v-for="(item,index) in diy" :key="index">
            <Row>
                <Col :span="24"><img :src="item.pic" alt="" style="width:85%;"></Col>
            </Row>
            <Row>
    ...
    <Row>
        <Col :span="8" v-for="(item,index) in mix_1" :key="index">
            <Row>
                <Col :span="24"><img :src="item.pic" alt="" style="width:90%;"></Col>
            </Row>
            <Row>
                <Col :span="24" class="font-12">{{item.title}}</Col>
            </Row>
    ...
    <Row v-for="(item,index) in mix_118" :key="index" class="pt-1 font-12">
        <Col :span="6"><img :src="item.pic_premium" alt="" style="width:100%;"></Col>
        <Col :span="14" class="pt-3 pl-2">
            <Row>
                <Col :span="24"><p class="float-left font-bold">{{item.title}}</p></Col>
            </Row>
            <Row>
                <Col :span="24"><p class="float-left">{{item.author}}</p></Col>
            </Row>
        </Col>
        <Col :span="4"><h2 style="line-height:80px;">...</h2></Col>
    </Row>
    ...
    <div v-for="(item,index) in mix_119">
        <Row>
            <Col :span="24"><img :src="item.pic" alt="" style="width:100%;"></Col>
        </Row>
        <Row>
            <Col :span="24"><p class="float-left pl-2">{{item.title}}</p></Col>
        </Row>
    </div>
...
export default {
  ...
  created(){
    axios.get(this.api+"http://musicapi.qianqian.com/v1/restserver/ting?from=android&version=6.9.1.0&channel=xiaomi&operator=0&method=baidu.ting.plaza.index&cuid=20FEE5C3E4D640433364068880D4F7C0&focu_num=8")
      .then(res=>{
          console.log(res)
